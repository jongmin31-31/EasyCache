...
읽기 전용 도메인의 경우, 자동 장애 조치 수행 과정에서 읽기 접근을 위한 클라이언트들의 통신을 방해하지 않기 위해 도메인 정보가 바로 갱신되지는 않으나 콘솔 화면에 읽기 전용 도메인 갱신 이라는 버튼이 노출되며 해당 버튼을 클릭하면 버튼을 누르는 시점을 기준으로 읽기 전용 노드들의 IP 정보를 기준으로 갱신됩니다.
